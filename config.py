...
# Create a new dictionary with only variable names and descriptions
TARGET_VARIABLES_DESCRIPTIONS = {key: value["description"] for key, value in TARGET_VARIABLES.items()}

CLUSTER_TARGET_VARIABLES = {
    "demographic_info": ["target_users", "n_participants", "adult_participant_age", "participant_gender", "participant_ethnicity"],
...
# Add clustered paragraph tag descriptions to PARAGRAPH_TAG_DESCRIPTIONS
for cluster_name, var_names in CLUSTER_TARGET_VARIABLES.items():
...
# Add "other" paragraph tag descriptions to PARAGRAPH_TAG_DESCRIPTIONS
